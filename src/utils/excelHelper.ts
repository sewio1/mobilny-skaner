...
export const exportToOriginalExcel = async (
  ...
  try {
    ...
    let cLokalizacja = colIndexLokalizacja;
    let cKodGlowny = colIndexKodGlowny;
    ...
    let cAdnotacje = colIndexAdnotacje;
    ...
    items.forEach((item) => {
      ...
      if (sheetRowCode !== itemCodeParsed && itemCodeParsed) {
        let found = false;
        worksheet.eachRow((r, rowIdx) => {
          if (rowIdx > headerRowIdx) {
            ...
            if (codeVal === itemCodeParsed && (locVal === itemLoc || !cLokalizacja)) {
              row = r;
              found = true;
            }
          }
      ...
      if (cAdnotacje) {
        row.getCell(cAdnotacje).value = item.adnotacje || null;
      }
